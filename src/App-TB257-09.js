...
function App() {
  ...
  const getDailyWord = () => {
    ...
    // -------------------------------------------------------------------------------------
    var wordQuery = 'https://0ggg6zbkf5.execute-api.us-east-1.amazonaws.com/test/nextword'
    // Delete the next line of code when you wire up the web service
    setDailyWord(wordQuery.data.body.result);
  };
  ...
  const handleSave = () => {
    let data = {
      ...
      playDate: 11222334455, // Add date here
      word: dailyWord,
      guesses: [
        firstGuess,
        secondGuess,
